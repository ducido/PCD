...
def main(cfg: DictConfig) -> None:
    ...
    heatmaps = load_iss(path, views=VIEW_NAMES)  # (261, 256, 256)

    ooi_path = os.path.join(cfg.data.root, f"episode_{cfg.data.episode}_ooi.gif")
    ooi_masks = imageio.mimread(ooi_path)
    masks_list = []
    for val in ooi_masks:
        masks_list.append(val[:,:,0])
    masks = {}
    masks['front'] = np.stack(masks_list, axis=0)
