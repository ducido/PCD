...
import gc, torch, torch._dynamo
from iss_nmr_toolkit.constants import DEFAULT_VIEWS


def reload_policy(policy, policy_cfg):
    policy.model.to('cpu')
    policy.model = None
    policy.env_adapter = None
    del policy
    gc.collect()
    torch._dynamo.reset()          # drop compiled graphs + guards
    gc.collect()
    torch.cuda.empty_cache()
    torch.cuda.synchronize()
    print(f"after free: alloc={torch.cuda.memory_allocated()/2**30:.2f}G "
          f"reserved={torch.cuda.memory_reserved()/2**30:.2f}G")
    return PiZeroContrastInference(**policy_cfg)

Observation = Dict[str, Any]
Heatmaps = Dict[str, np.ndarray]

# Order used to flatten the policy's per-DoF outputs into one action vector.
...
def pizero_infer(policy, chunk, batch_size, algo):

    chunk_results = []
    for obs in chunk:
        image = obs['image']
        contrast_image = obs['contrast_image']
        proprio = obs['states']
        instruction = obs['prompt']
        if algo == 'knn_topK_long_motion':
            _, actions, _ = policy.knn_topK_motion_step(image, contrast_image, instruction=instruction, proprio=proprio, M_action_horizon=4)
        elif algo == 'knn':
            print("knn")
            _, actions, _ = policy.knn_de_step(image, contrast_image, instruction=instruction, proprio=proprio) 
        elif algo == 'pcd':
            print("pcd")
            _, actions, _ = policy.pcd_step(image, contrast_image, instruction=instruction, proprio=proprio) 
        elif algo == 'base':
            print("base")
            _, actions = policy.baseline_step(image, instruction=instruction, proprio=proprio)


        action = np.concatenate([actions[0]["world_vector"], 
                                actions[0]["rot_axangle"], 
                                actions[0]["gripper"]])

        # actions_list = []
        # for ac in actions:
        #     action = np.concatenate([ac["world_vector"], 
        #                             ac["rot_axangle"], 
        #                             ac["gripper"]])
        #     actions_list.append(action)
        
        chunk_results.append({'actions': action})
    return chunk_results

from contrast_policies.pizero_contrast import PiZeroContrastInference

def run_inference(policy: Any, observations: Sequence[Observation], batch_size: int, desc: str, algo: str) -> list[dict[str, Any]]:
    ...
    chunk_results = pizero_infer(policy, observations, batch_size, algo)
...
def compute_iss(
    ...
    *,
    ...
    policy_cfg = dict(
        ...
        checkpoint_path='pretrained/open-pi-zero',
        num_repeats=24
    )
    policy = reload_policy(policy, policy_cfg)
    #############
    ...
    for step in step_iter:
        ...
        for chunk_idx in range(num_chunks):
            ...
            batch_results = pizero_infer(policy, batch_inputs, batch_size, algo)[:chunk_size]
