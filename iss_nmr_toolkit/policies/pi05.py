...
def _use_local_openpi_cache() -> None:
    """Serve the PaliGemma tokenizer from the repository instead of Google Cloud.

    OpenPI hardcodes ``gs://big_vision/paligemma_tokenizer.model`` and resolves it
    through ``OPENPI_DATA_HOME/<bucket>/<object>``, returning the local copy
    without touching the network when it already exists. Compute nodes have no
    outbound connectivity, so point the cache at ``checkpoints/openpi_tokenizer``
    whenever that copy is present.
    """
    cache_dir = Path(__file__).resolve().parents[2] / "checkpoints" / "openpi_tokenizer"
    if (cache_dir / "big_vision" / "paligemma_tokenizer.model").is_file():
