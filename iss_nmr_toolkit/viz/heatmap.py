...
class _H264Writer:
    """Write RGB frames to a VS Code/browser-friendly H.264 MP4 through pyav.

    The ffmpeg binary is not available in this environment, so encoding uses the
    libx264 encoder bundled with pyav instead of shelling out.
    """

    def __init__(self, path: Path, frame_shape: tuple[int, ...], fps: int) -> None:
        path.parent.mkdir(parents=True, exist_ok=True)
        height, width = frame_shape[:2]
        if height % 2 or width % 2:
            raise ValueError(f"H.264 yuv420p needs even frame dimensions, got {width}x{height}")
...
def render_iss_videos(
    ...
    *,
    ...
    writers: dict[str, _H264Writer] = {}
    view_names = list(views.keys())

    for view_name, obs_key in views.items():
        writers[view_name] = _H264Writer(output / f"{view_name}_iss.mp4", observations[0][obs_key].shape, fps)
